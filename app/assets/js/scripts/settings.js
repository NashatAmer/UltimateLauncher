    ...
    document.getElementById('settingsAddServerCode').onclick = () => {
        for(let ele of document.getElementsByClassName('settingsInputServerCodeVal')){
            ...
            if(!ConfigManager.getServerCodes().includes(code) && code){
                ...
                ConfigManager.save()
                loggerSettings.log('Ajout d\'un code de serveur')
                prepareLauncherTab()
            } else {
                loggerSettings.log('Le code du serveur existe déjà ou est vide, ne pas ajouter.')
            }
        }
    }
        ...
        v.onclick = () => {
            if(v.hasAttribute('remcode')){
                ...
                if(ConfigManager.getServerCodes().includes(code)){
                    ...
                    ConfigManager.save()
                    loggerSettings.log('Ajout dU code supprimé de la configuration et l\'a enregistré')
                    prepareLauncherTab()
                }
            }
            loggerSettings.log('Le code de serveur n\'existe pas .')
        }
    })
}
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le launcher, vous devez être connecté à au moins un compte. Vous devrez vous connecter à nouveau après. <br><br> Êtes-vous sûr de vouloir vous déconnecter?',
                    'Je suis sûr',
                    'Annuler'
                )
                setOverlayHandler(() => {
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Sélectionné: Java ${v.version.major} Mise à jour ${v.version.update} (x${v.arch})`
            } else {
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/vasolix/UltimateLauncher/releases.atom',
        success: (data) => {
